...
fig1 = go.Figure()
fig1.add_trace(go.Scatter(x=df[mask]['Incident Time'].sort_values().unique(), y=df[mask].groupby(['Incident Time']).mean()['Row ID'], line=dict(color='red', width=4)))
...
fig3 = go.Figure()
fig3.add_trace(go.Scatter(x=df[mask].sort_values(by=['Incident Datetime'])['Incident Datetime'].unique(), y=df[mask].groupby(['Incident Datetime']).count()['Row ID'], line=dict(color='red', width=4)))
...
fig4 = px.sunburst(df[mask], color_discrete_sequence = ['#c40000','#ff7676'], values='contador', path=['Police District','Analysis Neighborhood'], hover_name='Analysis Neighborhood')
